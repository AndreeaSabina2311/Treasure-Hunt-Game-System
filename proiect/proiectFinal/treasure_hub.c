...
struct sigaction sa; //structura pentru configurarea handler-ului de semnal
struct dirent *entry;
...
void handle_signal(int sig)
{
  ...
  if (sig == SIGUSR1) {
    int fd = open("monitor_cmd.txt", O_RDONLY); //deschide fisierul de comenzi
    if (fd<0) {
    ...
    int child=fork(); //creeaza proces copil pentru executarea comenzii
    if(child<0)
      {
    ...
    if(child==0)
	  ...
	  if(*hunt_id=='\0')
	    {
	      printf("Monitor: lipseste id-ul vanatorii pentru list_treasures\n");
	      exit(1);
	    }
	  ...
	  if(hunt && treasure_id)
	    {
	      ...
	      perror("Monitor: exec esuat pentru --view_treasure");
	      exit(1);
	    }
	  else
	    {
...
void setup_signal_handlers() //configureaza handler-ele pentru semnale
{
...
void monitor_loop()
{
  setup_signal_handlers();//activeaza functia care trateaza semnalele
...
void start_monitor()
{
  ...
  if(monitor_pid == 0)
    {
      monitor_loop(); //copil: executa bucla monitorului
      exit(0);
    }
  monitor_activ=1;
...
void send_command_to_monitor(const char *cmd) {
    ...
    dprintf(fd,"%s\n",cmd); //scrie comanda in fisier
    close(fd);

    
    kill(monitor_pid, SIGUSR1); //trimite semnalul catre monitor

    usleep(1000000);

    char buffer[2048];
    int n=read(pipefd[0], buffer, sizeof(buffer)-1); //citeste raspunsul din pipe
    if(n>0)
      {
	buffer[n]='\0';
	printf("%s\n", buffer);
      ...
      if(entry_local->d_type == DT_DIR && entry_local->d_name[0]!='.')
	{
	  ...
	  if(pid==0)
		...
		printf("%s", buf);
	      ...
	      close(pipe_local[0]);
	      waitpid(pid, NULL, 0); // așteaptă terminarea copilului curent
