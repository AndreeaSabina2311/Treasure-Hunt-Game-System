...
{
  //<2, deoarece minim 2 argumente sunt mereu
  if(argc<2)
    {
  ...
  // Verificăm dacă comanda este --list_hunts, care este un caz special ce nu necesită argv[2]
  if(strcmp(argv[1],"--list_hunts") == 0) {
    // Implementarea comenzii --list_hunts
    DIR *main_dir, *hunt_dir;
    char hunt_path[512];

    if ((main_dir = opendir(".")) == NULL)
    {
        perror("Nu s-a putut deschide directorul curent");
        exit(-1);
    }

    printf("Lista vanatorilor si numarul de comori:\n");
    ...
    while ((entry = readdir(main_dir)) != NULL)
    {
        ...
        if (entry->d_type == DT_DIR &&
            ...
            strcmp(entry->d_name, "..") != 0)
        {
            // Construim calea către subdirectorul vânătorii
            snprintf(hunt_path, sizeof(hunt_path), "./%s", entry->d_name);

            // Încercăm să deschidem subdirectorul
            hunt_dir = opendir(hunt_path);
            if (hunt_dir == NULL)
            {
                // Ignorăm acest director dacă nu putem să-l deschidem
                continue;
            }

            int treasure_count = 0;

            // Parcurgem fișierele din subdirectorul curent
            while ((hunt_entry = readdir(hunt_dir)) != NULL)
            {
                if (strstr(hunt_entry->d_name, ".dat") != NULL)
                {
                    // Construim calea completă către fișierul .dat
                    char treasure_file_path[768];
                    snprintf(treasure_file_path, sizeof(treasure_file_path), "%s/%s", 
                             hunt_path, hunt_entry->d_name);
                    
                    // Deschidem fișierul pentru a număra comorile
            ...
            // Afișăm numele vânătorii și numărul de comori
  ...
  //daca al doilea argument din linia de comanda este LIST_TREASURES
  ...
  //nume director, (total) file size si ultima modificare al fisierului(fisierelor) al comorii, apoi comorile
      ...
      while((entry=readdir(dir))!=NULL)
	{
      ...
      //logam actiunea
      char log_path[512];
      snprintf(log_path, sizeof(log_path), "./%s/logged_hunt.txt", argv[2]);
  ...
  //pasi:verific daca exista (daca nu exista ma opresc cu un perror si un exit); daca exista, parcurg directorul si sterg fiecare fisier, apoi sterg legatura simbolica, iar apoi sterg directorul
      ...
      if(stat(argv[2],&st)==-1) //testez daca exista, iar daca nu exista directorul,dau mesaj si ma opresc
	{
      ...
      else //daca exista directorul, ma plimb prin el si sterg fiecare fisier
	{
	  dir=opendir(path);
	  if(dir==NULL)
	    {
	  ...
	  closedir(dir);
	  if(unlink(symlink_path)==-1) //sterg legatura simbolica
	    {
	      perror("eroare stergere legatura simbolica");
	      exit(-1);
	    }
	  if(rmdir(path)==-1) //sterg directorul
	    {
