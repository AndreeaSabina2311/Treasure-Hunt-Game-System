...
void calculate_scores()
{
  ...
  while((entry=readdir(dir))!=NULL)
      ...
      if(entry->d_type == DT_DIR && strncmp(entry->d_name,"game",4)==0)
	{
	  ...
	  //creez procesul copil pentru functia de calcul a scorului
	  int pid=fork();
	  if(pid<0)
	    {
	      perror("Eroare la fork pentru creare copil");
	      continue;
	    }

	  if(pid==0) //a fost creat procesul copil
	    {
	      close(pfd[0]); //inchidem capatul de citire pentru a scrie in pipe
